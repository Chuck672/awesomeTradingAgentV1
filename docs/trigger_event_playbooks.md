...
站稳（close_outside）默认规则：
- M15：需要连续 2 根收盘确认
- M30 及以上：需要连续 1 根收盘确认

首次触及（first touch）默认规则：
- zone.last_touch_age_candles >= 10：视为第一次触及（新鲜区）
- zone.last_touch_age_candles < 10：视为多次触及（已被反复测试）
...
retest 缓冲带默认规则（exec_tf 的 ATR_14）：
- trend_breakout / break-retest：buffer ∈ [0.35, 0.60] * ATR_14(exec_tf)
- range 边界反转（贴边交易）：buffer 优先使用 level-zone edge，本文件默认不额外放大
...
当 rectangle_breakout 被“大震荡包裹”时：
- 使用 swing point 判定：最近 4 个 zigzag swing points 中，至少存在 1 个 swing high >= rectangle.top 且 1 个 swing low <= rectangle.bottom（即至少两个 swing 点从上下包裹住该 rectangle）
- 输出 hold 或 uncertain
...
- 趋势中继 vs 大震荡包裹：是否统一使用 swing point 包裹判定，还是需要叠加其它过滤（例如更高周期同样处于 Consolidation）？
