...
export interface MeasureStyle extends BaseDrawingStyle {
  fillColorUp: string;
...
class MeasurePaneRenderer {
  constructor(private _drawing: MeasureDrawing) {}

  draw(target: CanvasRenderingTarget2D): void {
    const points = this._drawing.getPoints();
    const style = this._drawing.getStyle() as MeasureStyle;
    ...
    target.useMediaCoordinateSpace((scope) => {
      ...
      const themeColor = isUp ? style.fillColorUp : style.fillColorDown;

      // Fill Background
      ctx.fillStyle = applyOpacity(themeColor, style.fillOpacity);
      ctx.fillRect(x, y, w, h);

      // Draw Center Arrow Line
      ...
      let bars = 0;
      if (p1.logical !== undefined && p2.logical !== undefined) {
        bars = Math.abs((p2.logical as number) - (p1.logical as number));
      }

      const text = `${priceDiff > 0 ? '+' : ''}${priceDiff.toFixed(2)} (${percentDiff.toFixed(2)}%) | ${bars} Bars`;
      
      ctx.font = '12px sans-secondary';
      const textMetrics = ctx.measureText(text);
      const textWidth = textMetrics.width + 12;
      const textHeight = 24;
      
      // Position text box near p2 but keep it inside screen if possible
      let textX = px2.x - textWidth / 2;
      let textY = isUp ? Math.min(px1.y, px2.y) - textHeight - 8 : Math.max(px1.y, px2.y) + 8;
      
      ctx.fillStyle = themeColor;
      ctx.beginPath();
      ctx.roundRect(textX, textY, textWidth, textHeight, 4);
      ctx.fill();
...
class MeasurePaneView implements IPrimitivePaneView {
  ...
  constructor(drawing: MeasureDrawing) {
    this._renderer = new MeasurePaneRenderer(drawing);
  }
  renderer() { return this._renderer; }
  zOrder(): 'normal' { return 'normal'; }
}

export class MeasureDrawing extends BaseDrawing {
  constructor(id?: string) {
    super('measure', { ...defaultMeasureStyle }, id);
