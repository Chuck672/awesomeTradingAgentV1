...
export interface HorizontalRayStyle extends BaseDrawingStyle {
  direction: 'left' | 'right';
}

export const defaultHorizontalRayStyle: HorizontalRayStyle = {
  lineColor: '#FF9800',
  lineWidth: 2,
  lineStyle: 'solid',
  direction: 'right'
...
class HorizontalRayPaneRenderer {
  constructor(private _drawing: HorizontalRayDrawing) {}

  draw(target: CanvasRenderingTarget2D): void {
    const points = this._drawing.getPoints();
    const style = this._drawing.getStyle() as HorizontalRayStyle;
...
export class HorizontalRayDrawing extends BaseDrawing {
  constructor(id?: string) {
    super('horizontal_ray', { ...defaultHorizontalRayStyle }, id);
  ...
  addPoint(point: DataPoint): boolean {
    // Horizontal Ray only needs 1 point
  ...
  hitTestCustom(pixel: PixelPoint, tolerance: number): HitTestResult | null {
    ...
    const style = this._style as HorizontalRayStyle;
    const dist = HitTestUtils.pointToHorizontalRayDistance(pixel, px1, style.direction);
    if (dist <= tolerance) {
