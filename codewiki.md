...
系统内部实际上并行运行着**两套**多智能体架构，分别针对不同的业务场景：一套是用于用户前端互动的**主系统 (3-Agent Supervisor 架构)**，另一套是专为后台行情监控设计的**次级系统 (Alert System / `event_dual` 双 Agent 架构)**。

### 2.1 主系统：互动式 3-Agent 编排模型

用于处理前端聊天框中用户主动发起的复合意图任务。
...
*   **🔍 Analyzer (分析 Agent)**：
    *   **职责**：负责“读”和“思考”。
    *   **机制**：采用典型的 ReAct 模式，被授权调用数据查询与指标计算工具箱。能够自主拉取 K 线数据、计算技术指标等，为交易决策提供严谨的数据支撑并给出量化评分。
*   **🛠️ Executor (执行 Agent)**：
    *   **职责**：负责“写”和“行动”。
    *   **机制**：仅被分配动作类工具，负责风险控制逻辑计算、在前端图表上执行物理绘制以及操控 UI 状态。

### 2.2 次级系统：告警专用的 `event_dual` 双 Agent 流水线

用于处理后台自动触发的监控告警（如结构破坏、趋势衰竭）。为了保证高频事件下的低延迟与高稳定性，采用了硬编码的**单向流水线** (`START -> analyzer -> executor -> END`)，并彻底移除了 Supervisor 路由节点。

*   **🔍 Analyzer (静态分析节点)**：
    *   **权限降级**：完全剥夺工具调用 (Tool Calling) 权限。
    *   **机制 (预计算注入)**：不自主查询数据，而是直接接收上游打包好的静态 JSON（包含多周期 K 线、预计算指标等）。只基于给定 JSON 进行纯逻辑推理，生成交易信号及进出场点位，并通过 Telegram 等即时发送报告。
*   **🛠️ Executor (可视化执行节点)**：
    *   **权限降级**：移除所有交易执行和风控工具权限。
    *   **机制**：唯一职责是解析 Analyzer 生成的结构化输出，调用绘图工具在前端图表上进行标注（如标记压力线、入场位），实现后台决策的前端可视化。
