...
from backend.services.chart_scene.indicators import detect_swings
...
def tool_structure_level_generator(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    payload:
      bars_by_tf: {tf:[bars]}
      level_generator: schema.structures.level_generator
      primary_timeframe: str
    returns:
      levels: [{price, kind, ...}]
      zones:  [{center, half_width_pips, ...}] (简化)
    """
    ...
    # 估算 bar 间隔（秒），用于把 zone_max_age_bars 转为 to_time
    ...
    # prev_day levels 的默认 from_time：当日 00:00 UTC（prev day 高低从当日开盘起有效）
            ...
            zones.append(
                {
                    "type": "zone",
                    "center": center,
                    "half_width_pips": float(zone_half_width),
                    "pip_size": float(pip_size),
                    "half_width": float(half),
                    "top": center + float(half),
                    "bottom": center - float(half),
    ...
    if payload.get("_debug"):
        ...
        return {
            ...
            "_debug": {
                ...
                "bars_by_tf_keys": list(bars_by_tf.keys())[:10],
                "bars_len_primary": len(bars) if isinstance(bars, list) else -1,
                "sample_bar0": (bars[0] if isinstance(bars, list) and bars else None),
                "prev_day_cnt": prev_cnt,
                "sources": sources,
            },
        }
